...
from spice.cli.commands import InteractiveCommandContext, SlashCommandRegistry, _select_from_choices
...
class UpwardCompletionPromptSession(PromptSession[str]):
    ...
    def _create_layout(self):
        ...
        has_before_fragments, get_prompt_text_1, get_prompt_text_2 = _pt_prompt._split_multiline_prompt(self._get_prompt)
        ...
        all_input_processors = [
            ...
            _pt_prompt.ConditionalProcessor(_pt_prompt.PasswordProcessor(), dyncond("is_password")),
            _pt_prompt.DisplayMultipleCursors(),
            _pt_prompt.DynamicProcessor(lambda: _pt_prompt.merge_processors(self.input_processors or [])),
            _pt_prompt.ConditionalProcessor(
        ...
        bottom_toolbar = _pt_prompt.ConditionalContainer(
            _pt_prompt.Window(
                _pt_prompt.FormattedTextControl(lambda: self.bottom_toolbar, style="class:bottom-toolbar.text"),
        ...
        search_toolbar = _pt_prompt.SearchToolbar(search_buffer, ignore_case=dyncond("search_ignore_case"))
        ...
        system_toolbar = _pt_prompt.SystemToolbar(enable_global_bindings=dyncond("enable_system_prompt"))
        ...
        default_buffer_window = _pt_prompt.Window(
            ...
            get_line_prefix=_pt_prompt.partial(self._get_line_prefix, get_prompt_text_2=get_prompt_text_2),
        ...
        completion_space_filter = _pt_prompt.has_focus(default_buffer) & has_completions & ~_pt_prompt.is_done
        main_input_content = _pt_prompt.HSplit(
            [
                _pt_prompt.ConditionalContainer(
                    _pt_prompt.Window(height=_pt_prompt.Dimension(min=1, preferred=COMPLETION_MENU_RESERVED_ROWS, max=COMPLETION_MENU_RESERVED_ROWS)),
                    filter=completion_space_filter,
                ),
                _pt_prompt.ConditionalContainer(
                    _pt_prompt.Window(_pt_prompt.FormattedTextControl(get_prompt_text_1), dont_extend_height=True),
                    _pt_prompt.Condition(has_before_fragments),
                ),
                _pt_prompt.ConditionalContainer(
                    default_buffer_window,
                    _pt_prompt.Condition(lambda: _pt_prompt.get_app().layout.current_control != search_buffer_control),
                ),
                _pt_prompt.ConditionalContainer(
            ...
            [
                _pt_prompt.Float(
                    ...
                    content=_pt_prompt.CompletionsMenu(
                        ...
                        scroll_offset=1,
                        extra_filter=_pt_prompt.has_focus(default_buffer) & ~multi_column_complete_style,
                    ),
                ),
                _pt_prompt.Float(
                    xcursor=True,
        ...
        layout = _pt_prompt.HSplit(
            [
                ...
                _pt_prompt.ConditionalContainer(_pt_prompt.ValidationToolbar(), filter=~_pt_prompt.is_done),
                _pt_prompt.ConditionalContainer(
                    system_toolbar,
                    dyncond("enable_system_prompt") & ~_pt_prompt.is_done,
                ),
                _pt_prompt.ConditionalContainer(
                    _pt_prompt.Window(_pt_prompt.FormattedTextControl(self._get_arg_text), height=1),
                    dyncond("multiline") & _pt_prompt.has_arg,
                ),
                _pt_prompt.ConditionalContainer(search_toolbar, dyncond("multiline") & ~_pt_prompt.is_done),
                bottom_toolbar,
            ]
        )
...
async def render_prompt(session: AgentSession, renderer: CliRenderer, prompt: str) -> None:
...
def _bottom_toolbar(session: AgentSession, renderer: CliRenderer, *, show_edit_hint: bool = False):
...
async def _handle_plan_ready(
    *,
    ...
    if choice == "auto":
        ...
        console.print("[green]Switched to edit mode.[/green] Executing with file edits auto-approved for this session.")
        await render_prompt(agent_session, renderer, prompt)
...
async def run_conversation(
    ...
    *,
    ...
    if not sys.stdin.isatty():
        console.print("[bold red]Error:[/bold red] interactive mode requires a terminal stdin.")
        console.print("Use [bold]spice run <prompt>[/bold] for non-interactive execution.")
        return
    ...
    while True:
        ...
        if message.startswith("/"):
            ...
            if result.session:
                ...
                commands = SlashCommandRegistry(agent_session.extensions)
                input_session.completer = SpiceInputCompleter(commands.completer(), cwd=Path.cwd())
            if result.clear_requested:
                ...
                console.print(f"[dim]Session: {agent_session.session_label}[/dim]")
            if result.prompt:
                await render_prompt(agent_session, renderer, result.prompt)
        ...
        if agent_session.plan_state.is_plan_mode and _is_execute_request(message):
            prompt = agent_session.approve_plan("manual")
            console.print("[green]Switched to edit mode.[/green] Executing the approved plan.")
            await render_prompt(agent_session, renderer, prompt)
            continue

        await render_prompt(agent_session, renderer, message)
        await _handle_plan_ready(
            console=console,
            input_session=input_session,
            renderer=renderer,
